...
The Camel JBang MCP Server now includes 21 runtime introspection tools (`camel_runtime_*`) for inspecting and
interacting with live Camel applications. These tools communicate with running Camel processes via the
file-based IPC protocol in `~/.camel/` and support process auto-discovery.

The IPC protocol has been enhanced to support multiple concurrent clients using request-ID-suffixed
action/output files (`{pid}-action-{requestId}.json` / `{pid}-output-{requestId}.json`). This is fully
backward compatible — existing CLI commands continue to use the single-file pattern.

See xref:camel-jbang-mcp.adoc[Camel MCP Server] for the full list of tools.
...
=== camel-mcp-server (new)

A new `camel-mcp-server` component provides an embedded MCP server for standalone Camel applications. When
enabled, it exposes Camel DevConsole data (context, routes, health, endpoints, tracing, etc.) as MCP tools
over the STDIO transport, allowing AI assistants to introspect the running application directly.

Configuration via `application.properties`:

[source,properties]
----
camel.main.mcpServer.enabled = true
camel.main.mcpServer.transport = stdio
camel.main.mcpServer.serverName = camel-mcp-server
----

The property `mcpDevConsoleEnabled` (default `true`) controls whether DevConsole-based tools are registered.
Use `includeTools` and `excludeTools` to filter which tools are exposed.
