...
[NOTE]
====
The build uses the Develocity Maven extension (`.mvn/extensions.xml`), which enables a *local*
build cache by default (`.mvn/develocity.xml`) so incremental local builds can reuse unchanged
module outputs. Remote caching is always disabled. If you rebuild a module and your changes don't
seem to take effect -- for example a jar in `~/.m2` doesn't reflect a source change you just
...
To disable it for every local build without changing the command line each time, set it once via
...
The opt-in `-Pdep-check` profile leverages https://github.com/maveniverse/pilot[`pilot:dependencies`] for bytecode-level detection of *used-but-undeclared* and *unused-but-declared* dependencies.
It is non-blocking by default (report mode), so it only prints findings without failing the build.

The mojo requires compiled classes.
Run `test-compile` first (or `package` for accurate test-scope analysis):

[source,bash]
----
# Full reactor — report mode (default)
mvn test-compile eu.maveniverse.maven.plugins:pilot-plugin:0.3.0:dependencies -Pdep-check -Dlicense.skip -Dquickly

# Skip test-scope analysis (faster)
mvn compile eu.maveniverse.maven.plugins:pilot-plugin:0.3.0:dependencies -Pdep-check -Dpilot.skipTestScope=true -Dlicense.skip -Dquickly
----
...
mvn test-compile eu.maveniverse.maven.plugins:pilot-plugin:0.3.0:dependencies -Pdep-check
----
...
[source,bash]
----
mvn test-compile eu.maveniverse.maven.plugins:pilot-plugin:0.3.0:dependencies -Pdep-check -Dpilot.action=check
----
